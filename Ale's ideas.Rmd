...
d <- recode_missing_df(selected_data) %>%
  ...
  select(hhid, child_sex, child_age, home_language, stories_songs, read, confident, how_well_demands, primary_cg_ed, state)
```
...
mod1 <- lm(confident ~ read, d)
mod2 <- lm(confident ~ stories_songs, d)
...
mods <- by_ed_level %>%
    mutate(m1 = map(data, ~lm(confident ~ read, data = .x)),
           m2 = map(data, ~lm(confident ~ stories_songs, data = .x)))
...
mods %>%
    mutate(comp12 = map2(m1, m2, anova),
           p_val_12 = map_dbl(comp12, extract_p)) # this didn't work, it says not meaningful for factors
```
