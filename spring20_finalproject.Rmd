...
library(english)
library(glue)
library(fs) #loaded to batload data 
...
#Jim
d1 <- full_join(NCSH, state)
#Just curious to see what it would look like with a full_join

#code to batch load data for practice 
...
names(batch)
#loaded the data with map_df to see if I can run the code the same way 
```
...
#Jim
more_missing <- c(45, 50, 54)
recode_missing(more_missing)
...
#practicing function on various data sets
recode_missing_df(iris)
recode_missing_df(mtcars)
recode_missing_df(batch) #nice, worked on batch dataset
```
