...
Great job writing functions! While if i'm interpreting it right, we want to replace any 90, 95, 99 in our dataset by NA, we can do below without writing functions. Just FYI.

```{r}
foo <- selected_data
foo[foo == 90 | foo == 95 | foo == 99] <- NA
dd <- foo %>% drop_na()
```

Just so you know that I have checked the following chunk with the dataframe "dd" I just created and obtained the same result.
