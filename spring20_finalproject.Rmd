...
First off, sorry for the late peer review. For some reason I thought these were due Wednesday. 

Areas of strength:
1.	I thought it was smart that you all changed the values for your variables. This was something I was interested in for our project since we weren’t sure to keep it to the codebook or give variables more meaningful names. I think after seeing this project, making the changes makes more sense.  
2.	I like the meaningful paths for your plots when saving them. Making it so that if someone wants to read just plot they can. 
3.	I think having a function for detecting missing values is a smart use of a function. I like that you have two functions, one for dataframes and the other for vectors. 

What I learned:
1.	I think the coolest thing I learned was using your function and then piping all the rest of your code into that function.

Areas of improvement:
1.	I’m not sure about using a sequence pattern for the scale of your plots. Since the values are your n, it may be better just to use a different color palette and include the n within the plots. I have made this change with some small changes to your code. 
2.	Your second plot was also kind of difficult to view from other states, so I facetted them, which you can see if you like it more. Its interesting that data is only available for some caregiver education. 
...
plot1 <- plot1_df  %>%
  ...
  mutate(plots = pmap(list(state, label, data),
                      ...
                      labs(title = glue("Confidence in School Preparedness Between \nLevels of Caregiver Education: {..1}"),
                             ...
                             caption = ..2) + 
                        facet_wrap(~confident, ncol = 1) +
                        geom_text(aes(label = prop_conf), hjust = -.50, size = 2) +
                        theme(plot.title.position = "plot",
                              legend.position = "none"))) #If you want to align your plot flush to the left margin
plot1$plots[[1]]
